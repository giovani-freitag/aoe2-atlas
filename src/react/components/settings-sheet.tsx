...
export function SettingsSheet({ civilizations, from, to, sliceYear, loading, open, onClose }: SettingsSheetProps) {
    ...
    return (
        <div className="sheet sheet--settings leather" ref={sheet} popover="auto" onToggle={(event) => {
            if (event.newState === 'closed') onClose();
        }} aria-label="Ajustes do mapa">
            ...
            <div className="sheet__body">
                ...
                <section className="card parchment singed">
                    ...
                    <p className="card__hint">
                        Nenhuma projeção é neutra. As áreas nos painéis são medidas na esfera quando os dados são
                        gerados, então trocar aqui muda o desenho e nenhum número.
                    </p>
                ...
                <p className="sheet__credit">
                    ...
                    (GPL-3.0) · costa de Natural Earth · maravilhas e emblemas da Age of Empires Series Wiki ·
                    geometria de <span className="numeric">{GENERATED_AT.slice(0, 10)}</span>
                </p>
            </div>
        </div>
    );
